...
function clampPosition(top: number, left: number) {
  ...
  return { top: Math.min(Math.max(8, top), Math.max(8, maxTop)), left: Math.min(Math.max(8, left), Math.max(8, maxLeft)) };
}
...
export function ColorPickerPopover({ open, anchorRef, value, onChange, onClose }: ColorPickerPopoverProps) {
  ...
  // Keep internal HSV/hex in sync when the upstream value changes (e.g. another swatch click).
  useEffect(() => {
    if (!open) return;
  ...
  useLayoutEffect(() => {
    ...
    setPosition(clampPosition(desiredTop, desiredLeft));
  }, [open, anchorRef]);
  ...
  const handleHexChange = (raw: string) => {
    ...
    if (rgb) {
      ...
      // Preserve hue when achromatic so the hue slider doesn't snap to 0 on white/black.
    ...
    >
      ...
      >
        <div
          ...
          style={{ left: `${hsv.s * 100}%`, top: `${(1 - hsv.v) * 100}%`, background: previewColor }}
        />
      </div>
      ...
      <div className="mt-3 flex items-center gap-2">
        <div
          aria-hidden
          className="h-7 w-7 shrink-0 rounded-[7px] border border-[var(--divider)]"
        ...
        <input
          ...
          spellCheck={false}
          className="mono min-w-0 flex-1 rounded-[7px] border border-[var(--divider)] bg-[var(--glass-thin)] px-2 py-1.5 text-[12px] uppercase tracking-wide text-[var(--text)] outline-none focus:border-[var(--accent-ring)]"
        />
